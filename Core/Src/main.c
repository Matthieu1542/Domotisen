...
uint32_t HUMI_SENSOR_GetValue(void);
uint32_t ACCEL_SENSOR_GetValue(void);
uint32_t ALARME_SENSOR_GetValue(void);
uint32_t MESURE_DISTANCE(void);
...
int main(void)
{
  ...
  printf("Test code\r\n");
    ...
    if (IKS01A3_ENV_SENSOR_Init(IKS01A3_HTS221_0, ENV_TEMPERATURE | ENV_HUMIDITY) != BSP_ERROR_NONE){
  	  printf("Erreur d'initialisation du capteur HTS221\n");
    }
    if (IKS01A3_MOTION_SENSOR_Init(IKS01A3_LIS2DW12_0, MOTION_ACCELERO) != BSP_ERROR_NONE){
  	  printf("Erreur d'initialisation du LSM6DSO\n");
    }
    ...
    IKS01A3_ENV_SENSOR_GetValue(IKS01A3_HTS221_0, ENV_HUMIDITY, &Humidite);
    IKS01A3_MOTION_SENSOR_GetAxes(IKS01A3_LIS2DW12_0, MOTION_ACCELERO, &accel_data);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin : PIR_SCREEN_Pin */
  GPIO_InitStruct.Pin = PIR_SCREEN_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(PIR_SCREEN_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pin : PIR_ALARME_Pin */
  GPIO_InitStruct.Pin = PIR_ALARME_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(PIR_ALARME_GPIO_Port, &GPIO_InitStruct);
...
uint32_t ACCEL_SENSOR_GetValue(void)
{
    ...
    // Calcul de la magnitude avec la norme euclidienne
    uint32_t magnitude = (uint32_t)sqrt((double)(accel_data.x * accel_data.x) + (double)(accel_data.y * accel_data.y) + (double)(accel_data.z * accel_data.z));
    return magnitude;
}

void delay_us(uint16_t us)
{
    __HAL_TIM_SET_COUNTER(&htim2, 0);  // remet le timer à 0
    while (__HAL_TIM_GET_COUNTER(&htim2) < us);  // attend le nombre de µs
}


uint32_t ALARME_SENSOR_GetValue(void)
{
	if(HAL_GPIO_ReadPin(GPIOH, PIR_ALARME_Pin)==1) {
		return 1;
	}else{
...
uint32_t MESURE_DISTANCE(void)
{
	...
	HAL_Delay(50);  // attendre que la capture se fasse
...
void Servo_SetAngle(TIM_HandleTypeDef *htim, uint32_t Channel, uint8_t angle) {
    if (angle > 180) angle = 180;  // Limiter à 180° max
    if (angle < 0) angle = 0;      // Limiter à 0° min

    // Conversion angle (0-180°) en largeur d'impulsion (1000-2000 µs)
    uint32_t pulse = 1000 + ((angle * 1000) / 180);

    // Modifier le rapport cyclique pour changer la position du servo
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	if(HAL_GPIO_ReadPin(GPIOI, PIR_SCREEN_Pin)==1) {
